...
class AttachmentController extends AdminController {
	...
	public function uploadJson(){
		...
		if(!$info) {
			...
			// 上传成功
			$downloadedfile = array('filename'=>$info['imgFile']['name'], 'filepath'=>$img_url, 'filesize'=>$info['imgFile']['size'], 'fileext'=>$info['imgFile']['ext'], 'authcode' => $info['imgFile']['md5'], 'savepath'=>$info['imgFile']['savepath'], 'savename'=>$info['imgFile']['savename']);
			D('Attachment')->saveData($downloadedfile, $dir);
			$this->ajaxReturn(array('error' => 0, 'url' => $img_url));
		}
	}
