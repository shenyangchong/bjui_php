...
class SystemController extends AdminController{
	...
	public function adminChangeStatus(){
		...
		//修改状态
		//dump($detail['status']);exit();
		//$status = $detail['status'] == 0 ? 1 : 0;//傻逼写法，三元运算适合表达式，不适合取反用
        $status = !$detail['status']+"";//更改状态值，取反
	...
	public function adminDelete(){
		...
		foreach ($userids as $userid){
			//过滤不需要删除的 角色 ID
			if($userid == 1){
				$this->ajaxReturn(array('statusCode'=>300,'message'=>'admin管理员不能删除','tabid'=>'System_adminManage'));
				continue;
		...
		$this->ajaxReturn(array('statusCode'=>200,'message'=>'管理员撤销成功','tabid'=>'System_adminManage'));
	}
    ...
    public function adminRoleForbid(){
    	...
    	$this->ajaxReturn(array('statusCode'=>200,'tabid'=>'System_adminRoleList','message'=>'状态更改成功！'));
    }
...
//     			}
    ...
    //菜单移动
    public function ajax_nodeMove(){
    	if(IS_POST){
    		$ids = I('post.ids');
    		$parentid = I('post.parentid', '', 'intval');
    		
    		//修改父级节点
    		$result = D('AdminMenu')->where(array('id' => array('in', $ids)))->save(array('parentid' => $parentid));
    		if($result){
    			$return['flag'] = true;
    		}else{
    			$return['flag'] = false;
